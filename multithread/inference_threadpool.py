...
def main(logname):
    ...
    log_full_name = os.path.join(config.save_path, config.save_info_fname)
    with open(log_full_name, "w") as fp:
        ...
        for test_name in test_names:
            ...
            npz_full_name = os.path.join(config.save_path, save_npz_name)
            np.savez(npz_full_name, refinement)
            os.chmod(npz_full_name, mode=0o444)
            print("`{}` saved and made read-only".format(npz_full_name))
            
            png_full_name = os.path.join(config.save_path, save_png_name)
            plt.imsave(png_full_name, refinement)
            os.chmod(png_full_name, mode=0o444)
            print("`{}` saved and made read-only".format(png_full_name))
            
            vis_full_name = os.path.join(config.save_path, save_img_name)
            plt.imsave(vis_full_name, vis_image)
            os.chmod(vis_full_name, mode=0o444)
            print("`{}` saved and made read-only".format(vis_full_name))
    ...
    os.chmod(log_full_name, mode=0o444)
    print("`{}` saved and made read-only".format(log_full_name))
